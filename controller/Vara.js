class VaraController {
  ...
  getAll = async (req, res) => {
    try {
      const varas = await this.service.getAll();
      res.status(200).json(varas);
    } catch (error) {
  ...
  getById = async (req, res) => {
    try {
      const { id } = req.params;
      const vara = await this.service.getById(Number(id));
...
import service from '../service/Vara.js';
export default new VaraController();